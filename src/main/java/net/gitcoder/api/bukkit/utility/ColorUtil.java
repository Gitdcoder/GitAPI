...
public class ColorUtil {
     ...
     */
    public String replaceColors(String toReplace, Object... args) {
        return replaceColors(String.format(toReplace, args));
    }

    public String replaceColorsSave(String toReplace, Object... args) {
        return String.format(replaceColors(toReplace), args);
    }

    public String toRainbow(String input) {
     ...
     * @return String with ChatColors to there associated position
     */
    public String replaceColors(String toReplace) {
        if (toReplace == null) {
            return "";
        }
    ...
    private String rainbow(String toReplace) {
    ...
    private int getNextPos(String str) {
    ...
    private String doRainbow(String toRainbow) {
    ...
    private Pattern COLOR_MATCH = Pattern.compile("(?i)" + String.valueOf('\u00A7') + "[0-9A-FK-OR]");

    public  String colorize(String input) {
        return ChatColor.translateAlternateColorCodes('&', input);
    }

    public String insertAll(String text, ChatColor replacement) {
        int size = StringUtil.charCount(text);
        ChatColor[] replace = new ChatColor[size];
        Arrays.fill(replace, replacement);
        return insert(text, replace);
    }

    private String insert(String text, ChatColor... replacement) {
        Matcher matcher = COLOR_MATCH.matcher(text);
        StringBuffer output = new StringBuffer();
        int counter = 0;
        while (matcher.find() && counter < replacement.length) {
            String s = matcher.group();
            ChatColor replace = replacement[counter++];
            if(replace == null) continue;
            matcher.appendReplacement(output, s.concat(replace.toString()));
        }
